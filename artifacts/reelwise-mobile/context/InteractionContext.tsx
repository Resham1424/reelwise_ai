...
const DEFAULT_STATE: InteractionState = { liked: ['java', 'lifestyle', 'interview'], skipped: [], rewatched: { java: 1 }, watchPercent: { java: 95, lifestyle: 91, interview: 88, laptop: 76 }, demoMode: true };
...
export function InteractionProvider({ children }: PropsWithChildren) {
  ...
  const value = useMemo<InteractionContextValue>(() => ({ ...state, likeReel: (id) => setState((current) => ({ ...current, liked: current.liked.includes(id) ? current.liked.filter((item) => item !== id) : [...current.liked, id] })), skipReel: (id) => setState((current) => ({ ...current, skipped: current.skipped.includes(id) ? current.skipped : [...current.skipped, id] })), rewatchReel: (id) => setState((current) => ({ ...current, rewatched: { ...current.rewatched, [id]: (current.rewatched[id] ?? 0) + 1 }, watchPercent: { ...current.watchPercent, [id]: 100 } })), advanceWatch: (id) => setState((current) => ({ ...current, watchPercent: { ...current.watchPercent, [id]: Math.min(100, (current.watchPercent[id] ?? 0) + 7) } })), toggleDemoMode: () => setState((current) => ({ ...current, demoMode: !current.demoMode })), resetDemo: () => setState(DEFAULT_STATE) }), [state]);
